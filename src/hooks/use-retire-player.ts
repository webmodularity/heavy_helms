...
export function useRetirePlayer(playerId: string) {
  ...
  const { refreshCharacters } = usePlayer();
   ...
   */
  const retirePlayer = async (): Promise<{ success: boolean; error?: Error }> => {
    ...
    try {
      ...
      const playerContractAddress = process.env.NEXT_PUBLIC_PLAYER_CONTRACT_ADDRESS as `0x${string}`;
      ...
      try {
        ...
        toast.success("Retirement request submitted", {
          description: "Your warrior retirement request has been submitted to the blockchain.",
          action: {
        ...
        try {
          ...
          toast.error("Transaction might be pending", {
            description: "Please check BaseScan and refresh the page in a moment.",
            duration: 5000,
          });
          return { success: false, error: error instanceof Error ? error : new Error("Transaction monitoring failed") };
        }
      } catch (txError) {
        console.error("Transaction request error:", txError);
        toast.error("Failed to send transaction", {
          description: txError instanceof Error ? txError.message : "Check console for details",
        });
        return { success: false, error: txError instanceof Error ? txError : new Error("Transaction request failed") };
      }
    } catch (error) {
