...
interface CompactNewCharacterCardProps {
  delay: number;
...
export function CompactNewCharacterCard({
  delay,
  onClick,
  isCreating,
  txHash,
}: CompactNewCharacterCardProps) {
  const [selectedNamePreference, setSelectedNamePreference] =
    useState<NamePreference>("male");
  ...
  return (
    <motion.div
      className={`relative rounded-lg overflow-hidden border-2 border-dashed border-yellow-700/40 bg-gradient-to-b from-black/30 to-black/10 flex flex-col justify-center items-center p-4 aspect-square cursor-pointer transition-all hover:bg-black/20 hover:border-yellow-700/60 ${
    ...
    >
      {isCreating ? (
        ...
        <>
          ...
          <h3 className="text-sm font-medium text-yellow-500 text-center mb-2">
            Create New Character
          </h3>
          <p className="text-xs text-center text-zinc-400 mb-3">
            Create a warrior to join the battle.
          </p>

          <fieldset
            className="flex flex-col items-center space-y-2 border-none p-0 m-0 w-full"
            onClick={(e) => e.stopPropagation()}
            onKeyDown={(e) => {
              e.stopPropagation();
            }}
          >
            <legend className="text-[10px] text-zinc-500 mb-1 text-center">
              Name Preference
            </legend>
